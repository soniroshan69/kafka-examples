...
object JoinOpKsToKs extends App {
  ...
  val rechargeReq: KStream[String, JsonNode] =
    builder.stream[String, JsonNode]("recharge-requests")(Consumed.`with`(new TimeExtractor)(stringSerde, jsonSerde))

  val rechargeConf: KStream[String, JsonNode] =
    builder.stream[String, JsonNode]("recharge-confirmations")(Consumed.`with`(new TimeExtractor)(stringSerde, jsonSerde))

  val completedTxn: KStream[String, String] = rechargeReq.join(rechargeConf)(
    (v1, v2) => if (v1.get("otp").asText().equals(v2.get("otp").asText())) "success" else "fail",
    JoinWindows.of(Duration.ofMinutes(5)))

  completedTxn.print(Printed.toSysOut())
